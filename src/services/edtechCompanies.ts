...
const log = createChildLogger('edtechCompanies');
...
export async function insertEdtechCompanies(companies: SmartRankingCompany[]): Promise<UpsertResult> {
  ...
  for (let i = 0; i < rows.length; i += BATCH_SIZE) {
    ...
    try {
      await knex('edtech_companies').insert(batch).onConflict('company_url').merge(UPDATABLE_COLUMNS);

      result.processed += batch.length;
    } catch (err) {
      log.error(
        { err, batchIndex, batchSize: batch.length, processedSoFar: result.processed },
        'Edtech companies batch upsert failed',
      );
      throw err;
    }
  }

  log.info(result, 'Edtech companies upserted');
  return result;
}
